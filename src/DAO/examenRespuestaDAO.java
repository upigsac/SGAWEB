...
public class examenRespuestaDAO {
    public boolean insertar(examenRespuestaC item) {
        ...
        try {
            c = ConexionWeb();
            cs = c.prepareCall(" {CALL DI.SP_MANTENIMIENTO_EXAMEN_RESPUESTA(1,?,?,?)}");
     ...
     public boolean eliminar(examenRespuestaC item) {
        Connection c ;
        CallableStatement cs ;
        boolean rpta = false;
        try {
            c = ConexionWeb();
            cs = c.prepareCall(" {CALL DI.SP_MANTENIMIENTO_EXAMEN_RESPUESTA(2,?,?,?)}");
            cs.setInt(1, item.getRespuesta());
            cs.setString(2, item.getDescripcion());
            cs.setInt(3, item.getEstadoRegistro());
